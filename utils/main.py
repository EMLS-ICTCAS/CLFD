...
sys.path.append(mammoth_path + '/pretrain')
...
def parse_args():
    ...
    if args.load_best_args:
        ...
        if "clfd" in args.model:
            model = args.model.replace('-clfd', '')
            model = model.replace('_clfd', '')
        else:
            model = args.model
...
def main(args=None):
    ...
    if "clfd" in args.model:
        backbone = dataset.get_DWT_backbone()
    else:
    ...
    if "clfd" in args.model:
        model = get_model(args, backbone, loss, dataset.get_DWT_transform())
    else:
