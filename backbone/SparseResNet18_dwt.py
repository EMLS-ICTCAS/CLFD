...
class DWT_SparseResNet(nn.Module):
    """
    ResNet network architecture. Designed for complex datasets.
    """

    def __init__(self, block: SparseBasicBlock, num_blocks: List[int],
                 num_classes: int, nf: int, kw_percent_on=0.1, kw_criterion='sum', kw_relu=False, wavename='haar') -> None:
        """
        Instantiates the layers of the network.
        :param block: the basic ResNet block
        :param num_blocks: the number of blocks per layer
        ...
        :param nf: the number of filters
        """
        super(DWT_SparseResNet, self).__init__()
        self.feature_extractor = feature_extractor(wavename=wavename)
        self.num_classes = num_classes
        self.nf = nf
        self.model_l = SparseResNet(block, num_blocks, num_classes, nf, kw_percent_on, kw_criterion, kw_relu)
        self.fz = 512
        self.classwise_select_counts = torch.zeros(num_classes, self.fz)
        self.select_probs = torch.zeros(self.fz)
        self.dropout_st = 0.6
        self.select_probs[:min(int(self.fz*1.1 * self.dropout_st),self.fz)] = 1
        self.classwise_select_probs = torch.zeros(num_classes, self.fz * 1)
        self.classifier = nn.Linear(self.fz, num_classes)



    def forward(self, x: torch.Tensor, y = None, retufull = False, disable_dropout=False, return_activations=False, mode='train', update_act_counts=True):
        """
        Compute a forward pass.
        :param x: input tensor (batch_size, *input_shape)
        :return: output tensor (output_classes)
        ...
        x = self.model_l(feature, y, disable_dropout=disable_dropout, return_activations=return_activations, mode=mode, update_act_counts=update_act_counts)

        if y is not None and not disable_dropout:
            ...
            x *= classwise_mask
        x = self.get_kvalue(x, y, self.dropout_st, disable_dropout)
    ...
    def construct(self, x: torch.Tensor, y = None, disable_dropout=False, return_activations=False, mode='train', update_act_counts=True):
        """
        Compute a forward pass.
        :param x: input tensor (batch_size, *input_shape)
        :return: output tensor (output_classes)
        """
        x = self.model_l(x, y, disable_dropout=disable_dropout, return_activations=return_activations, mode=mode,
                         update_act_counts=update_act_counts)
        x = self.get_kvalue(x, y, self.dropout_st, disable_dropout)
        out = self.classifier(x)
    ...
    def get_kvalue(self,x,y, k_percent_on, disable_dropout):
...
def DWT_sparse_resnet18(nclasses: int, nf: int=64, kw_percent_on=0.1, kw_criterion='sum', kw_relu=False, wavename = "haar") :
    """
    ...
    """
    return DWT_SparseResNet(SparseBasicBlock, [2, 2, 2, 2], nclasses, nf, kw_percent_on, kw_criterion, kw_relu, wavename = wavename)
